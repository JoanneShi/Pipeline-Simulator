...
int main(int argc, char *argv[]) {
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        newState.EXMEM.tempregA=state.IDEX.readRegA;
        newState.EXMEM.tempregB=state.IDEX.readRegB;
        
        
        if((opcode(state.EXMEM.instr)==ADD||opcode(state.EXMEM.instr)==NOR)&& // immediate
           ((state.EXMEM.destReg==field0(newState.EXMEM.instr))||
            (state.EXMEM.destReg==field1(newState.EXMEM.instr)))){
            if(state.EXMEM.destReg==field0(newState.EXMEM.instr)){
                newState.EXMEM.tempregA=state.EXMEM.aluResult;
                newState.EXMEM.regA=true;
            }
            if(state.EXMEM.destReg==field1(newState.EXMEM.instr)){
                newState.EXMEM.tempregB=state.EXMEM.aluResult;
                newState.EXMEM.regB=true;
            }
        }
        
        if((opcode(state.MEMWB.instr)==ADD||opcode(state.MEMWB.instr)==NOR||
                opcode(state.MEMWB.instr)==LW)&& // 1-away
                ((state.MEMWB.destReg==field0(newState.EXMEM.instr))||
                (state.MEMWB.destReg==field1(newState.EXMEM.instr)))){
            if((state.MEMWB.destReg==field0(newState.EXMEM.instr))&&!newState.EXMEM.regA){
                newState.EXMEM.tempregA=state.MEMWB.writeData;
                newState.EXMEM.regA=true;
            }
            if((state.MEMWB.destReg==field1(newState.EXMEM.instr))&&!newState.EXMEM.regB){
                newState.EXMEM.tempregB=state.MEMWB.writeData;
                newState.EXMEM.regB=true;
            }
        }
            
        if((opcode(state.WBEND.instr)==ADD||opcode(state.WBEND.instr)==NOR||
                opcode(state.WBEND.instr)==LW)&& // 2-away
                ((state.WBEND.destReg==field0(newState.EXMEM.instr))||
                (state.WBEND.destReg==field1(newState.EXMEM.instr)))){
            if((state.WBEND.destReg==field0(newState.EXMEM.instr))&&!newState.EXMEM.regA){
                newState.EXMEM.tempregA=state.WBEND.writeData;
                newState.EXMEM.regA=true;
            }
            if((state.WBEND.destReg==field1(newState.EXMEM.instr))&&!newState.EXMEM.regB){
                newState.EXMEM.tempregB=state.WBEND.writeData;
                newState.EXMEM.regB=true;
            }
        }
        ...
        if(opcode(newState.EXMEM.instr)==ADD){
            newState.EXMEM.destReg=field2(newState.EXMEM.instr)&0x7;
            newState.EXMEM.aluResult=newState.EXMEM.tempregA+newState.EXMEM.tempregB;
        } else if(opcode(newState.EXMEM.instr)==NOR){
            newState.EXMEM.destReg=field2(newState.EXMEM.instr)&0x7;
            newState.EXMEM.aluResult=~(newState.EXMEM.tempregA|newState.EXMEM.tempregB);
        } else if(opcode(newState.EXMEM.instr)==LW){
            newState.EXMEM.destReg=field1(newState.EXMEM.instr);
            newState.EXMEM.aluResult=newState.EXMEM.tempregA+state.IDEX.offset;
        } else if(opcode(newState.EXMEM.instr)==SW){
            newState.EXMEM.aluResult=newState.EXMEM.tempregA+state.IDEX.offset;
        } else if(opcode(newState.EXMEM.instr)==BEQ){
            if(newState.EXMEM.tempregA==newState.EXMEM.tempregB){
                newState.EXMEM.eq=1;
            } else{
                newState.EXMEM.eq=0;
            }
        ...
        newState.EXMEM.readRegA=newState.EXMEM.tempregA;
        newState.EXMEM.readRegB=newState.EXMEM.tempregB;
        newState.EXMEM.regA=false;
        newState.EXMEM.regB=false;
        
        /* --------------------- MEM stage --------------------- */
        
        newState.MEMWB.instr=state.EXMEM.instr;
        if(opcode(newState.MEMWB.instr)==ADD||opcode(newState.MEMWB.instr)==NOR){
            newState.MEMWB.destReg=field2(newState.MEMWB.instr)&0x7;
            newState.MEMWB.writeData=state.EXMEM.aluResult;
        } else if(opcode(newState.MEMWB.instr)==LW){
            newState.MEMWB.destReg=field1(newState.MEMWB.instr);
        ...
        } else if(opcode(newState.MEMWB.instr)==HALT){
        }
        ...
        if(opcode(newState.WBEND.instr)==ADD||opcode(newState.WBEND.instr)==NOR){
            newState.WBEND.destReg=field2(newState.WBEND.instr)&0x7;
            newState.reg[newState.WBEND.destReg]=state.MEMWB.writeData;
        }
        if(opcode(newState.WBEND.instr)==LW){
            newState.WBEND.destReg=field1(newState.WBEND.instr);//regB
            newState.reg[newState.WBEND.destReg]=state.MEMWB.writeData;
        } else{
            //ADD NOR SW BEQ HALT NOOP -- no action
        }
        ...
        /* this is the last statement before end of the loop. It marks the end
